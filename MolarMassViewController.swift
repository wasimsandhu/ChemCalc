...
class MolarMassViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return 1 + dictElementsAsArray.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (indexPath.row == 0) {
            ...
            let elementForCell = dictElementsAsArray[indexPath.row - 1]
            let numberForCell = dictQuantitiesAsArray[indexPath.row - 1]
            
            let massPercent = ((symbolMassDict[elementForCell]! * Double(numberForCell)) / molarMass!) * 100
